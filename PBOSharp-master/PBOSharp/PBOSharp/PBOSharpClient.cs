    ...
    public class PBOSharpClient : IDisposable
    {
        ...
        public PBO AnalyzePBO(string file)
        {
            try
            {
                ...
                PBOReader _reader = new PBOReader(stream, this);
        ...
        public void ExtractAll(PBO pbo, string extractionDir = null)
        {
            try
            {
                PushOnEvent($"Старт деобфускации и выгрузки файлов...", EventType.Debug);
                string outputDir = extractionDir ?? Path.Combine(Path.GetDirectoryName(pbo.LongName), "Decrypted_Output");

                int fakeFileCounter = 0;
                ...
                foreach (PBOFile file in pbo.Files)
                {
                    ...
                    // 1. Очищаем имя файла от знаков вопроса и недопустимых символов
                    safeFileName = safeFileName.Replace("?", "").Replace("\\\\", "\\");

                    // 2. Ловим и нейтрализуем запрещенные имена Windows (NUL, AUX, PRN, COM, LPT)
                    string[] forbiddenNames = { "NUL", "AUX", "PRN", "CON", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9" };
                    foreach (var forbidden in forbiddenNames)
                    {
                        if (safeFileName.Contains(forbidden))
                        {
                            // Заменяем запрещенное имя на безопасное, чтобы Windows разрешила создать файл
                            safeFileName = safeFileName.Replace(forbidden, "safe_data");
                        }
                    }

                    // 3. Обрабатываем мусорные папки вроде __JAPM__ и системные GUID {хххх-ххх}
                    if (safeFileName.StartsWith("__JAPM__") || safeFileName.Contains("{"))
                    {
                        fakeFileCounter++;
                        // Складываем подозрительные файлы в отдельную подпапку
                        safeFileName = Path.Combine("Unknown_Protected_Files", $"extracted_block_{fakeFileCounter}.bin");
                    }

                    // Формируем финальный безопасный путь на рабочем столе
                    string fullOutputPath = Path.Combine(outputDir, safeFileName);
                    ...
                    // Извлекаем байты
                    byte[] fileData = ExtractFileData(file);

                    if (fileData != null && fileData.Length > 0)
                    {
                        File.WriteAllBytes(fullOutputPath, fileData);
                        PushOnEvent($"Успешно обойдено и сохранено: {safeFileName}", EventType.Info);
                    }
                }

                PushOnEvent($"Готово! Все доступные блоки сохранены в: {outputDir}", EventType.Debug);
            }
            catch (Exception ex)
            {
                PushOnEvent($"Критическая ошибка файловой системы Windows: {ex.Message}", EventType.Error);
            }
        }
