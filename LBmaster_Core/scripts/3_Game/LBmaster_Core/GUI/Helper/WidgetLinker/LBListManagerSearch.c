class LBListManagerSearch<Class T1> : LBListManagerBase {

	typename TN_T1 = LBStringTools.FixTypename(T1);
	array<ref T1> arr;

	override bool IsSearched(int index, string search, array<Widget> extraWidgets) {
		return arr.Get(index).IsSearched(search, extraWidgets);
	}
	...
	void LBListManagerSearch(int copyIndexStart = 0, int copyIndexEnd = 0) {
